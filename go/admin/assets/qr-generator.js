/*
 * Browser bundle for the temporary environment QR shown by The North admin.
 *
 * QRCode for JavaScript copyright (c) 2009 Kazuhiko Arase, MIT License.
 * Node adaptation sourced from qrcode-terminal, Apache License 2.0.
 * This bundle adds a small SVG renderer and browser wrapper.
 * Full notices: THIRD-PARTY-NOTICES.txt
 */
(function (global) {
  'use strict';
  ...
  function localRequire(request) {
    var id = String(request || '').replace(/^\.\//, '').replace(/\.js$/, '');
    if (!Object.prototype.hasOwnProperty.call(modules, id)) {
      throw new Error('Unknown QR module: ' + request);
    }
  ...
  var QRCode = localRequire('index');
  var levels = localRequire('QRErrorCorrectLevel');
  ...
  function createSvg(text, options) {
    if (typeof text !== 'string' || text.length === 0) {
      throw new Error('A non-empty string is required to generate a QR code.');
    }

    options = options || {};
    var levelName = String(options.errorCorrection || 'H').toUpperCase();
    var level = Object.prototype.hasOwnProperty.call(levels, levelName) ? levels[levelName] : levels.H;
    var quietZone = positiveInteger(options.quietZone, 4);
    var foreground = cleanColor(options.foreground, '#061d43');
    var background = cleanColor(options.background, '#ffffff');

    var qr = new QRCode(0, level);
    qr.addData(text);
    qr.make();

    var count = qr.getModuleCount();
    var total = count + quietZone * 2;
    var path = [];
    ...
    for (var row = 0; row < count; row += 1) {
      ...
      while (col < count) {
        ...
        var run = col - start;
        path.push('M' + (start + quietZone) + ' ' + (row + quietZone) + 'h' + run + 'v1h-' + run + 'z');
      }
    }
  ...
  function createSvgDataUri(text, options) {
    return 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(createSvg(text, options));
  }

  global.TNQRCode = Object.freeze({
    createSvg: createSvg,
    createSvgDataUri: createSvgDataUri
  });
})(window);
